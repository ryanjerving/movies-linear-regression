...
And the impact on residuals is to essentially eliminate the heteroscedasticity we've seen in the low-end fan shape of our general model when comparing predicted to actual variables. Note that a regression line added to indicate the predicted/actual relationship is all but identical with our reference zero line.
...
A model designed around Comedy doesn't improve on the general model's $R^2adj.$ value of $0.5864$ quite as much as our Mystery & Suspense model does, though it comes in at a respectably improved $R^2adj.$ value of $0.6536$. But what's interesting is that it mobilizes a somewhat different set of significant variables, with anything Oscar-nomination related gone, release year (DVD or theatrical) only *just* significant, and the surprising appearance of `runtime` as a highly significant variable with a negative coefficient. The soul of wit is, indeed, brevity, it turns out, with comedies in danger of overstaying their welcome!
...
And as with Mystery & Suspense, the distribution of our Comedy model's residuals across the range of prediction scores flattens out the heteroscedasity we saw when comedies were buried within the general model. Here, the variation of predictive error holds largely steady across the range, suggesting this generically specific model is, in fact, a more useful model than then general.
...
TO DO: So here we are. What have we learned? What would we want to learn next?
Other ways of chopping up films? Are older films harder to predict than newer?
