...
But it's hard to see that here. And even zeroing in on the residual scatter of just two genres -- drama and comedy -- within the whole, there is nothing much apparent that jumps out when looked at across the range of observations in dataset order. 
...
But to look at the residuals across the range of `audience_score` predicted by our model by genre reveals something quite different, and instructive, particularly when taking the question of heteroscadisticity in account. We can see, for example, that the scores for dramas lean toward the higher end of the scale, but follow the basic pattern of the overall dataset in terms of increasing in predictive accuracy at the higher end as well. 
...
For comedies, on the other hand, the scores tend toward the lower end, but the predictions stand out from the general patten in that they do not show much improvement as scores rise, remaining broadly scattered into the 60s.  
...
# Train and optimize a model for the "mystery" sub-dataset
...
The particular collection of significant variables didn't change much -- critics' score, number of votes on IMDB, year of release (both DVD & theatrical), "Best Picture" nomination -- but the balance of each provided by the coefficient did. And the impact on residuals is to:

```{r residuals-mystery-model}

# Add predicted values and residuals to model
# Don't use below - use from first residuals blocks above
...
# Then you can use this same one for comedy.
