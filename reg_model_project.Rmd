...
On the one hand, our model fares pretty well in terms of linearity, as shown in the normal distribution within a histogram of the residual differences between the predicted actual values for `audience_score` across the `r m %>% summarise(count = n())` observations included in our final linear model.
...
# Show the normal distribution of residual values generated by the linear model 

gp <- ggplot(m, aes(x = residual))
ph <- geom_histogram(binwidth = 5, alpha = 0.4, color = pal[5], fill = pal[4])
th <- theme_minimal()
lb <- labs(x = NULL, y = NULL, title = "Count of Predicted Minus Actual Residual Values in Final Predictive Model")
...
This is confirmed by the random scatter of residual values around zero across our collection of observations (in the presumably random order presented by our dataset).
...
th <- theme_minimal()
lb <- labs(x = "Observations (in dataset order)", y = "Residual (Predicted minus Actual)", title = "Distribution of Residuals across Observations")
col <- scale_fill_brewer(palette = "Set3")
...
However, the fan shape we see in the residual values when we looked at them across the range of our model's predicted values suggests there is some reason to be concerned by the heteroscedasticity of our model: simply put, our residuals are greater -- and our model less successful at predicting `audience_score` -- at the lower ranges of predicted audience scores. That is to say, our model finds it harder to predict how much audiences will say they like the movies that they don't particularly like.
...
hl <- geom_hline(yintercept = 0, color = "black")
th <- theme_minimal()
lb <- labs(x = "Predicted Audience Score", y = "Residual (Predicted minus Actual)", title = "Distribution of Residuals across Range of Predicted Values")

gp + pp + hl + th + lb

```

TO DO: This points us back to our third research question: "Will the model we build work equally well for different kinds of movies?" In other words, given the different reasons that audiences seek out and enjoy different kind of movies, is there any reason to think the same set of predictors and the weight given to each would equally successfully target both documentaries and action movies or any other genre?

TO DO: Even more problematic if we start to bring other factors like genre into the picture. Here's what our model's residuals look like, broken out categorically by genre through color.

```{r residuals-vs-predicted-all-genres}

# Set some constants we'll use for the next set of charts

pt <- geom_jitter(alpha = 0.8)
hl <- geom_hline(yintercept = 0, color = "Black")
th <- theme_minimal()
...
TO DO: We know from earlier: We saw variation in audience score standard deviation earlier that might suggest our model's predictive power could vary sig. by genre, though Hard to see at levels of all genre (full set, all colors)

TO DO: And even looking for the particular contributions to residual scatter of just two genres -- drama and comedy -- there is nothing much apparent that jumps out when these are printed out across the range of observations in dataset order (other that what appears to be a slightly greater residual spread for comedy). 
...
TO DO: bUT LOOKING at the residuals by fitted reveals that the residuals for Drama vs. Comedy within the whole set of residuals look quite different, and have different levels of heteroscedastity. The values for `audience_score` for Drama lean toward the higher end of the scale, and follow the pattern of the overall residuals at the higher end to cluster more closely to the predicted values. 
...
# This color is bad -- will also want labels for all of these
...
TO DO: The values for `audience_score` among comedies, on the other hand lean toward the lower end of the scale, and suffer from greater unpredictability within our model as well. 
...
TO DO: An even greater disparity can be seen between the higher, and more predictably so, audience scores for documentary films within our model...
...
TO DO: ...and the lower, and highly unpredictable scores of action films.
...
```{r sources-for-visualization-ideas}
...
TO DO: all this just to say that genre may matter quite and bit
...
LET'S NOT AND SAY WE DID: So let's see how we can do with training a model on at least two of our genres and then testing each model on the other genre's data. 

OR MAYBE LET'S DO A LITTLE AS A TEASER
...
Indeed, simply building an optimized model for A COUPLE each of our genres with at least 30 movies -- Comedy AND Mystery & Suspense -- suggests the way they pose different challenges for the set of dimensions our dataset gives us to work with, with some genre-specific models able to outperform our all-movie model considerably (a model refined for "Mystery & Suspense" has an $R^2adj.$ value of $0.7258$).
...
tO DO: It may well be the case that a different mix of explanatory variables would better explain preferences among comedy audiences: perhaps greater recency of release means more for funny, or even something like a film's length.

eND WITH NEED FOR FURTHER REFINEMENT AND RESEARCH.
